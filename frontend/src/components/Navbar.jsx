            ...
            <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between gap-6">
                ...
                <div className="hidden md:flex items-center gap-10 text-sm">
                    {navItems.map((item) => (
                        ...
                        >
                            {item.label}
                            {/* Efek Garis Bawah Futuristik (Glow) */}
                            <span className={`absolute left-1/2 -translate-x-1/2 bottom-0 h-[2px] bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.8)] transition-all duration-300 ${
                ...
                <div className="flex items-center gap-6">
                    <Link to="/cart" className="relative group p-2 text-gray-300 hover:text-amber-300 transition-colors">
                        ...
                        {cart.length > 0 && (
                            <span className="absolute 0 -right-1 bg-amber-500 text-black text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-[0_0_10px_rgba(245,158,11,0.8)] animate-pulse">
                                {cart.length}
                            </span>
                        )}
                    </Link>
                    {user ? (
                        ...
                        <Link to="/login" className="text-xs tracking-widest uppercase px-6 py-2.5 rounded-full border border-amber-500/50 text-amber-300 hover:bg-amber-500 hover:text-black hover:shadow-[0_0_15px_rgba(245,158,11,0.5)] transition-all duration-500">
                            Login
                        </Link>
                    )}
                </div>
